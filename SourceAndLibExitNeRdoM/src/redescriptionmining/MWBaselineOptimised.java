 ...
 * @author matej
 */
public class MWBaselineOptimised {
   ...
   */
        ...
        for(int j=i+1; j<allSetsIncomplete.size();j++){
           /* if(j!=(i+1))
            ...
            for(int z=0;z<MWSet.redescriptions.size();z++){
                ...
                int num= MWSet.redescriptions.get(z).checkMerge(allSetsIncomplete.get(j).redescriptions.get(z1));//add function checkMerge to reduce the number of unecessarily created redescriptions
                for(int zz=0;zz<num;zz++){//returns 1 or 2 (how many reds possible to create)
                        tmp1 = MWSet.redescriptions.get(z).computeStatisticsAndReturn(allSetsIncomplete.get(j).redescriptions.get(z1),zz,dats, maps, appset.minJS, appset.minSupport);//simulate compute statistics and create red only if OK
                            if(tmp1!=null){    
                                if(tmp1.JS>=appset.minJS && tmp1.elements.size()>=appset.minSupport)
                                      tmpRd.add(tmp1);
                            }
                     }
           ...
           for(int z=0;z<allSetsIncomplete.get(i).redescriptions.size();z++){
                ...
                int num = allSetsIncomplete.get(i).redescriptions.get(z).checkMerge(allSetsIncomplete.get(j).redescriptions.get(z1));
               // System.out.println("Merge complete");
                for(int zz=0;zz<num;zz++){
                     tmp1 = allSetsIncomplete.get(i).redescriptions.get(z).computeStatisticsAndReturn(allSetsIncomplete.get(j).redescriptions.get(z1), zz, dats, maps, appset.minJS, appset.minSupport);
                    if(tmp1!=null){
                     if(tmp1.JS>=appset.minJS && tmp1.elements.size()>=appset.minSupport)
                        tmpRd.add(tmp1);
                     }
   ...
   // for(int i = MWSet.redescriptions.size()-1;i>=0;i--)
    ...
    for(int i = 0;i<MWSet.redescriptions.size()-1;i++){
        if(traversed.contains(i))
            continue;
        for(int j = i+1;j<MWSet.redescriptions.size();j++){
            if(js.computeRedescriptionElementJacard(MWSet.redescriptions.get(i), MWSet.redescriptions.get(j)) == 1.0)
                toRemove.add(j);
        }
